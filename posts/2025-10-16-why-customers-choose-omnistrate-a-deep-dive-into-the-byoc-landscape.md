...
| **Cloud Support** | Supports automated provisioning inside customer AWS or Azure accounts. **No GCP support.** | Support for AWS, GCP, Azure, AirGapped with OCI support coming soon. |
...
| **Tenant Model** | Install-centric — each deployment is not tenant-aware. No tenant auth, RBAC, or tenant management. No self-serve distribution or tenant-aware upgrades/day-2 operations. | Tenant-aware — from deployments, infrastructure, upgrades, billing to day-2 operations. |
| **Customer Experience** | No place for your customers to access, provision, monitor, configure, or govern their deployments and usage. | Unified customized portal, CLI, and API for your customers to access, provision, monitor, configure, and govern deployments, with enterprise auth and access control. |
...
| **Licensing & Entitlements** | No licensing or entitlement system. Controlled by CI/CD integration and Terraform access. | **Built-in licensing and entitlement engine** with auto-rotation, plan enforcement, usage-based metering, and expiration control. |
| **Infra Requirements** | Developers must define IaC modules and maintain state management. | **No Terraform or IaC required** — register container or Helm chart; can still use your own Terraform modules. |
| **AI Workload Support** | None except using TF. | **Native AI support** including GPU driver management, multi-tenancy, time-slicing/MIG, agent-based deployments, observability, governance, and outcome-based billing. |
